...
from filtering_service.swagger_service.apply_swagger_auto_schema import (
    ...
    def post(self, request, *args, **kwargs):
        ...
        if isinstance(data, list):
            for item in data:
                ...
                user_data_dict = {
                    field: item.get(key, None) for field, key in zip(fields, keys)
                }
                ...
                with transaction.atomic():
                    user_data.save()
            return Response(
                {"message": "Данные успешно сохранены"}, status=status.HTTP_201_CREATED
            )
        else:
            return Response(
                {"error": "Неверный формат данных"}, status=status.HTTP_400_BAD_REQUEST
            )


UserDataView = apply_swagger_auto_schema(tags=["upload data"], excluded_methods=[])(
    UserDataView
)
